...
public class SparkMySQL {
    public static void main(String[] args) {
        ...
        // config("spark.sql.shuffle.partitions", "3") 设置 shuffle 时的默认分区数
        // 没有硬编码 master 地址，这意味着运行模式完全由 spark-submit 时的 --master 参数决定，更灵活

        Dataset<Row> df = spark.read()
                .option("header", "true")
                .option("inferSchema", "true")
                .csv(SqlConfig.HDFS_PATH);
        // 从 HDFS 读取 CSV 文件，存入一个 Dataset<Row> 中
        // hdfsPath 指向 HDFS 上的一个 CSV 文件。
        ...
        // 将 DataFrame 注册为一个临时视图 sleep_table，之后就可以用标准 SQL 语句来查询它
